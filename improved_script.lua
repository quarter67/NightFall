-- BUILD: 2026-05-27-MOBILE-TOUCH-FIX13
...
if not allowRun then
    warn("[NightFall] Run loader.lua to start NightFall (key or keyless).")
    return
end
...
local function refreshShiftLockState()
    if State.shiftLockAutoSynced then return end
    if State.isMobile or State.freecamEnabled or State.spectating then
...
print("[NightFall] Loaded - build 2026-05-27-MOBILE-MOVE-FIX12")
if State.isMobile then
...
if State.isPremium then
    ...
    print("[NightFall] Premium locked - use a key in loader for A-Train Kill")
end
