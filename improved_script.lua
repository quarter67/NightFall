do
    ...
    if type(key) ~= "string" or key == "" then
        warn("[NightFall] Use the official loader to enter your key.")
        return
    end
end
...
pcall(function()
    ...
    local overlay = playerGui and playerGui:FindFirstChild("NightFallLoaderOverlay")
    if overlay then overlay:Destroy() end
    overlay = game:GetService("CoreGui"):FindFirstChild("NightFallLoaderOverlay")
    if overlay then overlay:Destroy() end
end)
...
-- GUI parent — CoreGui is what every executor uses for ScreenGuis.
...
local State = {
    ...
    trackedConnections = {},
    capturedDefaultSpeed = false,
    guiScale = 1.0,
    lastTouchScreenPos = nil,
    mobileFlightUp = false,
    mobileFlightDown = false,
    hideMobileGui = false,
    failsafeEnabled = false,
    failsafeThreshold = 25,   -- teleports when HP % drops BELOW this value
    failsafeTripped = false,  -- prevents spam until health recovers
    failsafeReturnCFrame = nil,
    aimbotDelayMs = 0,        -- 0 = instant lock; higher = smoother/slower
...
local GUI_SCALE_PATH  = "ScriptHub/gui_scale.txt"
local HIDE_MOBILE_PATH = "ScriptHub/hide_mobile_gui.txt"
local FAILSAFE_PATH     = "ScriptHub/failsafe.txt"
local AIMBOT_PATH       = "ScriptHub/aimbot.txt"

-- Safe zone coordinates (manual TP + failsafe destination)
local SAFE_ZONE_CFRAME = CFrame.new(296.5682678222656, 22.201051712036133, -255.1030731201172)
...
local function saveAimButtonPos(pos)
    fsWrite(AIM_POS_PATH, string.format("%s,%s,%s,%s",
...
local function loadGuiScale()
    local saved = tonumber(fsRead(GUI_SCALE_PATH))
...
local function applyGuiScale(scale)
    ...
    pcall(function() fsWrite(GUI_SCALE_PATH, tostring(State.guiScale)) end)
end
...
local function loadHideMobileGui()
    local data = fsRead(HIDE_MOBILE_PATH)
    return data == "1" or data == "true"
end

local function applyHideMobileGui(hidden)
    State.hideMobileGui = hidden and true or false
    -- Toggle the entire mobile-overlay ScreenGuis instead of individual elements
    -- so even if they get re-shown elsewhere, they stay invisible.
    if UI.MobileAimGui then UI.MobileAimGui.Enabled = not State.hideMobileGui end
    if UI.FovGui       then UI.FovGui.Enabled       = not State.hideMobileGui end
    pcall(function()
        fsWrite(HIDE_MOBILE_PATH, State.hideMobileGui and "1" or "0")
    end)
end

local function loadFailsafeSettings()
    local data = fsRead(FAILSAFE_PATH)
...
local function saveFailsafeSettings()
    pcall(function()
        fsWrite(FAILSAFE_PATH, (State.failsafeEnabled and "1" or "0") .. "," .. tostring(State.failsafeThreshold))
    end)
end
...
local function loadAimbotSettings()
    local data = fsRead(AIMBOT_PATH)
    if not data then return end
    local delay, part = data:match("([^,]+),([^,]+)")
    if delay then
...
local function saveAimbotSettings()
    pcall(function()
        fsWrite(
            AIMBOT_PATH,
            tostring(State.aimbotDelayMs or 0) .. "," .. tostring(State.aimbotTargetPart or "Head")
        )
    end)
end
...
UI.MobileAimGui.Enabled = true  -- always enabled; element Visible flags control what shows
...
FovGui.Enabled = true  -- always enabled; Visible flag controls display
...
local function switchTab(name)
    ...
    if name == "Misc" and State.__bundleA and State.__bundleA.refreshMiscPlayerList then
        State.__bundleA.refreshMiscPlayerList()
    end
end
...
AimbotDesc.Text = "0 ms delay = instant lock. Pick any character part name to track."
...
FailsafeDesc.Text = "Teleports to safe zone when your HP drops below the set % (not at exactly that %)."
...
A.updateMovementHacks = updateMovementHacks
A.updateFlight = updateFlight
A.setNoclip = setNoclip
A.updateSpeedHack = updateSpeedHack
A.resetMovementSettings = resetMovementSettings
A.stopDesyncEngine = stopDesyncEngine
A.setDesync = setDesync
A.setupCharacterMovement = setupCharacterMovement
A.applyJumpBoost = applyJumpBoost
A.applyJumpStats = applyJumpStats
A.applyMovementStatsNow = applyMovementStatsNow
A.stopFlight = stopFlight
...
F.beginCameraDrag = beginCameraDrag
F.endCameraDrag = endCameraDrag
F.applyCameraDragDelta = applyCameraDragDelta
F.updateFreecam = updateFreecam
F.stopFreecam = stopFreecam
F.setFreecam = setFreecam
F.updateBloodManipulator = updateBloodManipulator
F.updateBloodManipEffectBlock = updateBloodManipEffectBlock
F.stopBloodManipEffectBlock = stopBloodManipEffectBlock
F.resetBloodManipState = resetBloodManipState
F.setRemoveBloodManipEffects = setRemoveBloodManipEffects
F.updatePlayerESP = updatePlayerESP
F.refreshPlayerESPScan = refreshPlayerESPScan
F.updateFailsafe = updateFailsafe
F.teleportToSafeZone = teleportToSafeZone

end -- scope block 2 (Luau local register limit)

do -- scope block 2b (aimbot + wiring · Luau local register limit)
...
local function getAimbotSmooth()
    local delayMs = math.clamp(tonumber(State.aimbotDelayMs) or 0, 0, 500)
    if delayMs <= 0 then return 1 end
    return math.clamp(1 - (delayMs / 500) * 0.85, 0.12, 1)
end

local function getAimbotCameraRate()
    local delayMs = math.clamp(tonumber(State.aimbotDelayMs) or 0, 0, 500)
    if delayMs <= 0 then return 160 end
    return math.clamp(160 - (delayMs / 500) * 150, 8, 160)
end
...
local function getScreenMousePosition(useScreenCenter)
...
local function worldToScreen(worldPos)
    local cam = refreshCamera()
    if not cam or not worldPos then return nil end

    -- Use WorldToViewportPoint. Do NOT add GuiInset — it's unreliable in
    -- executor environments (inflated by executor CoreGui elements).
    -- Only reject targets behind the camera (Z <= 0); the `onScreen` bool can
    -- spuriously be false for targets near screen edges, causing misses.
    local ok, vpPos = pcall(function()
        local vp, _ = cam:WorldToViewportPoint(worldPos)
        return vp
    end)

    if not ok or not vpPos or vpPos.Z <= 0 then return nil end
...
local function getCharacterAimPart(character)
    ...
    local part = findAimPartInCharacter(character)
    if not part then
        return nil, nil
    end
...
local function getPlayerAimPart(plr)
    ...
    return getCharacterAimPart(plr.Character)
end

local function getPartAimWorldPosition(part)
    if not part or not part:IsA("BasePart") then return nil end
    if part.Name == "Head" then
        return part.CFrame.Position - Vector3.new(0, part.Size.Y * 0.3, 0)
    end
    return part.CFrame.Position
end

local function getAimMousePosition(useScreenCenter)
    return getScreenMousePosition(useScreenCenter)
end
...
local function getPartScreenPositionFromPart(part)
    if not refreshCamera() or not part or not part.Parent then
        return nil
    end

    local worldPos = getPartAimWorldPosition(part)
...
local function isValidAimTarget(plr)
    ...
    return getPartScreenPositionFromPart(part) ~= nil
end

local function getPartScreenPosition(plr)
    local part = getPlayerAimPart(plr)
    if not part then
        return nil
    end

    return getPartScreenPositionFromPart(part)
end

local function getMouseClosestTarget(useScreenCenter, maxFov)
    local refPos = getAimMousePosition(useScreenCenter)
    local closest, closestPart, closestDist = nil, nil, maxFov or AIMBOT_MAX_FOV
    
    pcall(function()
        for _, plr in pairs(Players:GetPlayers()) do
            if isValidAimTarget(plr) then
                local part = getPlayerAimPart(plr)
                local partScreen = getPartScreenPositionFromPart(part)
                if partScreen then
                    local dist = (partScreen - refPos).Magnitude
                    if dist < closestDist then
                        closestDist = dist
                        closest = plr
                        closestPart = part
...
-- Sticky lock with a grace window:
--   * While the locked target's part exists, stay on them (no retargeting).
--   * If the part disappears for less than AIMBOT_LOST_GRACE seconds, hold the
--     lock and skip aiming this frame (handles brief respawn flicker / cull).
--   * If the head stays gone past the grace window, the target died/disconnected:
--     drop the lock and immediately try to acquire a new target so the aimbot
--     keeps working without you toggling LOCK ON off and on again.
local function updateAimbotLock(useScreenCenter)
    local now = os.clock()
    ...
    if State.aimbotLockedTarget then
        ...
        if not State.aimbotHeadLostAt then
            State.aimbotHeadLostAt = now
        end

        if now - State.aimbotHeadLostAt < AIMBOT_LOST_GRACE then
            -- still in grace window, hold the lock and skip aiming this frame
            return State.aimbotLockedTarget, nil
        end

        -- grace expired: clear the lock and fall through to re-acquire
        State.aimbotLockedTarget = nil
        State.aimbotLockedHead = nil
        State.aimbotHeadLostAt = nil
        -- intentionally NOT setting aimbotLockExpired — auto-retarget instead
    end

    local target, part = getMouseClosestTarget(useScreenCenter)
    State.aimbotLockedTarget = target
    State.aimbotLockedHead = part
    State.aimbotHeadLostAt = nil
    return target, part
end

-- Predict where the part will be in `dt` seconds based on current velocity.
local function getPredictedPartWorldPos(part, dt)
    local base = getPartAimWorldPosition(part)
    if not base then return nil end

    local velocity
    local ok = pcall(function() velocity = part.AssemblyLinearVelocity end)
    if not ok or not velocity then
        ok = pcall(function() velocity = part.Velocity end)
    end
    if velocity and dt and dt > 0 then
        return base + velocity * dt
    end
    return base
end

-- Rotate the camera to look at a world position (mobile LOCK ON path).
local function aimCameraAtWorldPos(worldPos, dt)
    if not refreshCamera() or not worldPos then return false end

    local alpha = 1 - math.exp(-getAimbotCameraRate() * (dt or (1 / 60)))
...
local function aimMouseAtHead(part, useScreenCenter, dt)
    if not part then return false end
    ...
    local mousePos = getAimMousePosition(false)
    local delta = partScreen - mousePos
    if delta.Magnitude < 1 then
        return true
    end

    local smooth = getAimbotSmooth()
    local moved = moveMouseRelative(delta.X * smooth, delta.Y * smooth)
    if not moved then
        return aimCameraAtWorldPos(worldPos, dt)
    end
    return moved
...
UI.AimbotToggle.MouseButton1Click:Connect(function()
    ...
    -- When aimbot is turned off, clear the LOCK ON toggle state too
    if not State.aimbotEnabled then
        State.holdingMobileAim = false
        State.aimbotLockedTarget = nil
        State.aimbotLockedHead = nil
...
print("✅ NightFall loaded — build 2026-05-28-UNIFIED-FIX")
